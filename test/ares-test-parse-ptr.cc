...
TEST_F(LibraryTest, ParsePtrReplyOK) {
  ...
  EXPECT_EQ(HNS_SUCCESS, hns_parse_ptr_reply(data.data(), data.size(),
...
TEST_F(LibraryTest, ParsePtrReplyMalformedCname) {
  ...
  struct hostent *host = nullptr;
  EXPECT_EQ(HNS_EBADRESP, hns_parse_ptr_reply(data.data(), data.size(),
                                                addrv4, sizeof(addrv4), AF_INET, &host));
...
TEST_F(LibraryTest, ParsePtrReplyErrors) {
  ...
  data = pkt.data();
  EXPECT_EQ(HNS_ENODATA, hns_parse_ptr_reply(data.data(), data.size(),
                                               addrv4, sizeof(addrv4), AF_INET, &host));
  ...
  for (size_t len = 1; len < data.size(); len++) {
    EXPECT_EQ(HNS_EBADRESP, hns_parse_ptr_reply(data.data(), len,
                                                  addrv4, sizeof(addrv4), AF_INET, &host));
    EXPECT_EQ(nullptr, host);
  }
...
TEST_F(LibraryTest, ParsePtrReplyAllocFailSome) {
  ...
  for (int ii = 1; ii <= 18; ii++) {
    ClearFails();
    SetAllocFail(ii);
    EXPECT_EQ(HNS_ENOMEM, hns_parse_ptr_reply(data.data(), data.size(),
...
TEST_F(LibraryTest, ParsePtrReplyAllocFailMany) {
  ...
  for (int ii = 1; ii <= 63; ii++) {
    ClearFails();
    SetAllocFail(ii);
    int rc = hns_parse_ptr_reply(data.data(), data.size(),
                                  addrv4, sizeof(addrv4), AF_INET, &host);
    if (rc != HNS_ENOMEM) {
