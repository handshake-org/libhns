...
TEST_F(LibraryTest, ParseMxReplyOK) {
  ...
  struct hns_mx_reply* mx = nullptr;
  EXPECT_EQ(HNS_SUCCESS, hns_parse_mx_reply(data.data(), data.size(), &mx));
  ASSERT_NE(nullptr, mx);
  EXPECT_EQ("mx1.example.com", std::string(mx->host));
  EXPECT_EQ(100, mx->priority);

  struct hns_mx_reply* mx2 = mx->next;
...
TEST_F(LibraryTest, ParseMxReplyMalformed) {
  ...
  struct hns_mx_reply* mx = nullptr;
  EXPECT_EQ(HNS_EBADRESP, hns_parse_mx_reply(data.data(), data.size(), &mx));
...
TEST_F(LibraryTest, ParseMxReplyErrors) {
  ...
  // TODO(drysdale): check if this should be HNS_ENODATA?
  pkt.answers_.clear();
  ...
  pkt.answers_.clear();
  data = pkt.data();
  EXPECT_EQ(HNS_ENODATA, hns_parse_mx_reply(data.data(), data.size(), &mx));
  EXPECT_EQ(nullptr, mx);
  pkt.add_answer(new DNSMxRR("example.com", 100, 100, "mx1.example.com"));
  ...
  for (size_t len = 1; len < data.size(); len++) {
    int rc = hns_parse_mx_reply(data.data(), len, &mx);
    EXPECT_EQ(nullptr, mx);
...
TEST_F(LibraryTest, ParseMxReplyAllocFail) {
  ...
  for (int ii = 1; ii <= 5; ii++) {
    ClearFails();
    SetAllocFail(ii);
    EXPECT_EQ(HNS_ENOMEM, hns_parse_mx_reply(data.data(), data.size(), &mx)) << ii;
  }
}
